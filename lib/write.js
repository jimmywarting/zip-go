  ...
  flush (ctrl) {
    ...
    this.filenames.forEach((fileName) => {
      ...
      if (file.zip64 || file.offset > MAX_VALUE_32BITS) {
        needsZip64 = true
        // ZIP64 extra field: 2 (id) + 2 (size) + 8 (uncompressed) + 8 (compressed) [+ 8 (offset if needed)]
        const needsOffset = file.offset > MAX_VALUE_32BITS
        extraFieldLen = needsOffset ? 28 : 20
      }
    ...
    this.filenames.forEach((fileName) => {
      ...
      if (file.zip64 || file.offset > MAX_VALUE_32BITS) {
        // ZIP64 extra field
        // When offset exceeds 32-bit, we need to include all three fields:
        // uncompressed size (8), compressed size (8), and offset (8)
        const needsOffset = file.offset > MAX_VALUE_32BITS
        const extraDataSize = needsOffset ? 24 : 16
        
        dv.setUint16(extraIndex, 0x0001, true) // ZIP64 extra field tag
        dv.setUint16(extraIndex + 2, extraDataSize, true) // size of extra field data
        dv.setBigUint64(extraIndex + 4, file.uncompressedLength, true)
        dv.setBigUint64(extraIndex + 12, file.compressedLength, true)
        
        if (needsOffset) {
          dv.setBigUint64(extraIndex + 20, file.offset, true)
        }
        
        // Update extra field length in header (including 4-byte header)
        dv.setUint16(index + 30, extraDataSize + 4, true)
        extraIndex += extraDataSize + 4
      }
      
      data.set(file.comment, extraIndex)
      
      // Calculate the actual bytes written for this entry
      const entrySize = extraIndex - index + file.comment.length
      index += entrySize
    })
